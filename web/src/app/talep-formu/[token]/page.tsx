...
type Pub = {
  ...
  total: number | null; unit_price: number | null; price_visible: boolean; payment_terms: string | null; address: string | null;
  customer_name: string | null; company_signature: string | null; customer_signature: string | null;
...
export default function PublicAtfPage() {
  ...
  const load = useCallback(async () => {
    ...
    catch (e) { setErr(e instanceof ApiError ? e.message : "Form bulunamadı."); }
  }, [token]);
  useEffect(() => { load(); }, [load]);
  ...
  const Spec = ({ k, v }: { k: string; v: string }) => <p className="text-slate-600"><b className="text-slate-800">{k}:</b> {v}</p>;
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right text-sm">
              <p className="text-xs uppercase tracking-wide text-slate-400">ATF No</p>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm">
              <p className="mb-1 text-xs text-slate-400">Asansör Spesifikasyonu</p>
            ...
            <>
              <div className="mt-4 overflow-hidden rounded-xl border border-slate-200">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    <tr className="border-b border-slate-100"><td className="px-3 py-2">Birim Fiyat (Asansör Başına)</td><td className="px-3 py-2 text-right tabular-nums text-slate-600">{money(c.unit_price, c.currency)}</td></tr>
                    <tr><td className="px-3 py-2">Genel Toplam ({c.elevator_count ?? 1} adet)</td><td className="px-3 py-2 text-right tabular-nums font-semibold text-indigo-600">{money(c.total, c.currency)}</td></tr>
                  </tbody>
                </table>
              </div>
              <p className="mt-2 text-right text-xs text-slate-400">Tutarlar KDV dahildir.</p>
            </>
          )}

          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs text-slate-400">Garanti</p><p className="font-bold">{c.warranty_years ?? "—"} yıl</p></div>
            <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs text-slate-400">Teslim</p><p className="font-bold">{c.delivery_days ?? "—"} iş günü</p></div>
            <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs text-slate-400">Ödeme</p><p className="text-sm text-slate-600">{c.payment_terms ?? "—"}</p></div>
          </div>

          <div className="mt-6 space-y-4">
            {c.rendered_clauses.map((cl, i) => (
              <div key={i}><h3 className="text-sm font-semibold">{cl.title}</h3><p className="mt-1 text-sm leading-relaxed text-slate-600">{cl.body}</p></div>
            ))}
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-8 border-t border-slate-200 pt-6 text-center text-sm">
            ...
            <div>
              <p className="mb-2 text-xs text-slate-400">Talep Eden İmzası</p>
