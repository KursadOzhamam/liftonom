...
type Pub = {
  ...
  renewal_notice_days: number; document_status: string; terms: string | null;
  company_signature: string | null; customer_signature: string | null; customer_name: string | null;
...
export default function PublicContractPage() {
  ...
  const load = useCallback(async () => {
    try { setC(await api<Pub>(`/public/contracts/${token}`, { auth: false })); }
    catch (e) { setErr(e instanceof ApiError ? e.message : "Sözleşme bulunamadı."); }
  }, [token]);
  useEffect(() => { load(); }, [load]);
  ...
  function moveP(e: React.PointerEvent<HTMLCanvasElement>) {
  ...
  async function approve() {
    ...
    setBusy(true);
    try { await api(`/public/contracts/${token}/approve`, { method: "POST", body: { signature: sig }, auth: false }); setDone(true); await load(); }
  ...
  const approved = c.document_status === "approved" || done;
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-8 text-slate-800">
      <div className="mx-auto max-w-3xl px-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right text-sm">
              <p className="text-xs uppercase tracking-wide text-slate-400">Sözleşme No</p>
              <p className="text-lg font-bold">{c.contract_number}</p>
              <p className="mt-1 text-xs text-slate-500">{dt(c.start_date)} – {dt(c.end_date)}</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm">
              ...
              {c.building && <p className="text-xs text-slate-500">Bina: {c.building.name}</p>}
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm">
              <p className="font-semibold">{c.type ?? "Bakım"} Sözleşmesi</p>
              <p className="text-slate-600">Periyot: {c.period ?? "—"}{c.annual_visits ? ` · Yıllık ${c.annual_visits} ziyaret` : ""}</p>
              <p className="mt-1 font-semibold text-indigo-600">{money(c.monthly_fee, c.currency)}</p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-4">
            {c.rendered_clauses.length > 0 ? c.rendered_clauses.map((cl, i) => (
              ...
              <div><h3 className="text-sm font-semibold">Sözleşme Şartları</h3>
        ...
        {approved ? (
          <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-center text-emerald-700">
            ✓ Sözleşme onaylandı. Teşekkür ederiz.
          </div>
        ) : (
          <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-sm font-semibold">Sözleşmeyi Onayla</h3>
            ...
            <div className="relative mt-3">
              <canvas ref={canvasRef} width={320} height={140}
                onPointerDown={down} onPointerMove={moveP} onPointerUp={() => (drawing.current = false)} onPointerLeave={() => (drawing.current = false)}
            ...
            <button onClick={approve} disabled={busy}
              className="mt-3 w-full rounded-lg bg-indigo-600 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-60">
              {busy ? "Gönderiliyor…" : "İmzala ve Onayla"}
            </button>
          </div>
        )}
        <p className="mt-4 text-center text-xs text-slate-400">Bu sayfa {c.company?.name} tarafından gönderilmiştir.</p>
      </div>
    </div>
  );
}
