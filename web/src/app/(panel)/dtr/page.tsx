...
import { Plus, Eye, FileDown, Pencil, Trash2, Star } from "lucide-react";

type Row = {
  id: number; quote_number: string | null; status: string; created_at: string;
  elevator_name: string | null; inspector_name: string | null;
};
...
export default function DtrPage() {
  ...
  const loadTplCount = useCallback(async () => {
    try { const r = await api<{ data: Tpl[] }>("/quotes/templates?kind=dtr"); setTplCount(r.data.length); } catch { /* ignore */ }
  }, []);
  useEffect(() => { loadTplCount(); }, [loadTplCount]);

  return (
    <div>
      <h1 className="text-2xl font-bold text-ink">{tab === "docs" ? "Durum Tespit Raporları" : "Durum Tespit Raporu Şablonları"}</h1>
      <p className="mt-1 text-sm text-muted">TSE/G muayene sonrası durum tespiti.</p>
...
function DocsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      p.set("type", "dtr"); p.set("per_page", "50");
  ...
  async function del(id: number) {
    if (!(await confirm("Bu rapor silinsin mi?", { danger: true }))) return;
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-5">
      <div className="flex flex-wrap items-start gap-3">
        <input className="input flex-1" placeholder="Rapor no, muayene eden, asansör…" value={search} onChange={(e) => setSearch(e.target.value)} />
        <Link href="/dtr/new" className="btn-primary shrink-0"><Plus size={16} /> Yeni</Link>
      </div>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <Sel label="Durum" value={status} onChange={setStatus} opts={[{ v: "", l: "Tüm Durumlar" }, ...STATUSES]} />
        <Sel label="Sırala" value={sort} onChange={setSort} opts={[{ v: "", l: "En yeni tarih" }, { v: "valid_asc", l: "Tarihe göre" }]} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-line text-left text-xs uppercase tracking-wide text-muted">
              ...
              <th className="px-4 py-3 font-medium">Muayene Eden</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-10 text-center text-muted">Henüz rapor yok.</td></tr>
            ) : rows.map((r) => (
              <tr key={r.id} className="border-b border-line last:border-0 hover:bg-surface">
                ...
                <td className="px-4 py-3 text-ink-soft">{r.inspector_name ?? "—"}</td>
                <td className="px-4 py-3"><Badge status={r.status} /></td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/dtr/${r.id}/preview`} className="btn-primary px-2.5 py-1.5 text-xs"><Eye size={14} /> Önizle</Link>
                    <button onClick={() => downloadFile(`/quotes/${r.id}/pdf`, `${r.quote_number ?? `dtr-${r.id}`}.pdf`)} className="btn-ghost px-2.5 py-1.5 text-xs" title="PDF"><FileDown size={14} /></button>
                    <Link href={`/dtr/${r.id}/edit`} className="btn-ghost px-2.5 py-1.5 text-xs"><Pencil size={14} /> Düzenle</Link>
...
function TemplatesTab({ onChange }: { onChange: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const p = new URLSearchParams();
      if (search.trim()) p.set("search", search.trim());
      p.set("sort", sort); p.set("kind", "dtr");
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-5">
      ...
      <div className="mt-4 flex justify-end">
        <Link href="/dtr/templates/new" className="btn-primary"><Plus size={16} /> Yeni</Link>
      </div>
            ...
            {loading ? (
              ...
              <tr key={t.id} className="border-b border-line last:border-0 hover:bg-surface">
                <td className="px-4 py-3 font-medium text-ink">
                  <button onClick={() => router.push(`/dtr/templates/${t.id}`)} className="hover:text-primary">{t.name}</button>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/dtr/templates/${t.id}`} className="btn-ghost px-2.5 py-1.5 text-xs"><Pencil size={14} /> Düzenle</Link>
