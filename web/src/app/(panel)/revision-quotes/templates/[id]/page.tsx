...
const PLACEHOLDERS = [
  ...
  "asansor_no", "iscilik", "malzeme", "tutar", "belge_tarihi", "gecerlilik",
];

export default function RevisionTemplateEditorPage() {
  ...
  const [clauses, setClauses] = useState<Clause[]>([{ title: "1. Revizyon Kapsamı", body: "", active: true }]);
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const body = { name: name.trim(), kind: "revision", type: "Revizyon Teklifi", is_default: isDefault, is_active: isActive, clauses, variables };
      if (isNew) await api("/quotes/templates", { method: "POST", body });
      else await api(`/quotes/templates/${id}`, { method: "PUT", body });
      router.push("/revision-quotes");
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-ink">{isNew ? "Yeni Revizyon Teklifi Şablonu" : "Revizyon Teklifi Şablonu"}</h1>
      ...
      <div className="mt-5 rounded-2xl border border-line bg-card p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-6">
          <label className="flex items-center gap-2 text-sm text-ink-soft">
            ...
            Varsayılan şablon (yeni revizyon teklifi kaydında ön-seçili gelir)
          </label>
          <label className="flex items-center gap-2 text-sm text-ink-soft">
