...
const PLACEHOLDERS = [
  "firma_adi", "firma_vkn", "firma_adres", "firma_telefon", "musteri_adi",
  "teklif_no", "teklif_basligi", "teklif_tarihi", "gecerlilik", "tutar",
];
...
export default function QuoteTemplateEditorPage() {
  ...
  const [clauses, setClauses] = useState<Clause[]>([{ title: "1. Teklif Kapsamı", body: "", active: true }]);
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const body = { name: name.trim(), type: "Teklif", is_default: isDefault, is_active: isActive, clauses, variables };
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-ink">{isNew ? "Yeni Teklif Şablonu" : "Teklif Şablonu"}</h1>
      ...
      <div className="mt-5 rounded-2xl border border-line bg-card p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-6">
          <label className="flex items-center gap-2 text-sm text-ink-soft">
            ...
            Varsayılan şablon (yeni teklif kaydında ön-seçili gelir)
          </label>
          <label className="flex items-center gap-2 text-sm text-ink-soft">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <p className="text-xs text-primary">Kapak, son-sayfa görselleri ve filigran <b>Ayarlar → Belge Görselleri</b> bölümünden firma geneli yönetilir; seçtiğiniz tüm belge türlerinin PDF&apos;lerine otomatik uygulanır.</p>
        <div className="flex shrink-0 gap-2">
          <Link href="/quotes" className="btn-ghost">İptal</Link>
