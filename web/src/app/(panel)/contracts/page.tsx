...
type ContractDetail = {
  customer_id: number | null; type: string | null; start_date: string | null;
  end_date: string | null; monthly_fee: number | null; auto_renew: boolean;
...
export default function ContractsPage() {
  ...
  async function openEdit(id: number) {
    try {
      const d = await api<ContractDetail>(`/contracts/${id}`);
      setForm({
        customer_id: d.customer_id != null ? String(d.customer_id) : "",
        type: d.type ?? "Bakım",
        start_date: d.start_date ?? "", end_date: d.end_date ?? "",
        monthly_fee: d.monthly_fee != null ? String(d.monthly_fee) : "",
        auto_renew: !!d.auto_renew, status: d.status ?? "active", notes: d.notes ?? "",
      });
  ...
  async function save() {
    ...
    try {
      ...
      if (editId == null) await api("/contracts", { method: "POST", body });
      else await api(`/contracts/${editId}`, { method: "PUT", body });
  ...
  return (
    <div>
                ...
                <tr key={c.id} className="border-b border-line last:border-0 hover:bg-surface">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(c.id)} className="text-muted hover:text-primary" title="Görüntüle / Düzenle"><Pencil size={16} /></button>
      ...
      {modal && (
        <Modal title={editId == null ? "Yeni Sözleşme" : "Sözleşme Düzenle"} onClose={() => setModal(false)} footer={
          <>
