...
const PLACEHOLDERS = [
  "firma_adi", "firma_vkn", "firma_adres", "firma_telefon", "musteri_adi", "bina_adi", "bina_adres",
  "yonetici_adi", "baslangic", "bitis", "sozlesme_no", "periyot", "yillik_ziyaret", "tutar", "yenileme_gun",
];
...
export default function TemplateEditorPage() {
  ...
  const [type] = useState("Bakım Sözleşmesi");
  const [isDefault, setIsDefault] = useState(false);
  const [isActive, setIsActive] = useState(true);
  const [clauses, setClauses] = useState<Clause[]>([{ title: "1. Taraflar", body: "", active: true }]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const t = await api<{ name: string; is_default: boolean; is_active: boolean; clauses: string; variables: string }>(`/contracts/templates/${id}`);
        ...
        const vs = JSON.parse(t.variables || "[]") as Variable[];
        setVariables(vs);
  ...
  const removeClause = (i: number) => setClauses((c) => c.filter((_, j) => j !== i));
  const addClause = () => setClauses((c) => [...c, { title: `${c.length + 1}. Yeni Madde`, body: "", active: true }]);

  async function save() {
    if (!name.trim()) { alert("Şablon adı zorunludur."); return; }
    setSaving(true);
    try {
      const body = { name: name.trim(), type, is_default: isDefault, is_active: isActive, clauses, variables };
      if (isNew) await api("/contracts/templates", { method: "POST", body });
      else await api(`/contracts/templates/${id}`, { method: "PUT", body });
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-ink">{isNew ? "Yeni Bakım Sözleşmesi Şablonu" : "Bakım Sözleşmesi Şablonu"}</h1>
      ...
      <div className="mt-5 rounded-2xl border border-line bg-card p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-6">
          <label className="flex items-center gap-2 text-sm text-ink-soft">
            ...
            Varsayılan şablon (yeni sözleşme kaydında ön-seçili gelir)
          </label>
          <label className="flex items-center gap-2 text-sm text-ink-soft">
        ...
        <div className="mt-6 rounded-xl border border-line bg-surface p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {PLACEHOLDERS.map((p) => (
              <code key={p} className="rounded-md border border-line bg-card px-2 py-0.5 text-xs text-primary">{`{{${p}}}`}</code>
            ))}
          </div>
        </div>
      </div>
            ...
            <div key={i} className="rounded-xl border border-line p-4">
              <div className="flex items-center gap-2">
                ...
                <label className="flex shrink-0 items-center gap-1.5 text-xs text-ink-soft">
                  <input type="checkbox" checked={cl.active} onChange={(e) => setClause(i, { active: e.target.checked })} /> Aktif
                </label>
                ...
                <button onClick={() => removeClause(i)} className="btn-danger px-2 py-1.5" title="Sil"><X size={14} /></button>
              </div>
